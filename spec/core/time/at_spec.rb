...
describe "Time.at" do |it| 
  it.can "convert to time object" do
    # the #chomp calls are necessary because of RSpec
    Time.at(1168475924).inspect.chomp.should_equal(localtime(1168475924).chomp)
  end
  
  it.creates "a new time object with the value given by time" do
    t = Time.now
    Time.at(t).inspect.should_equal(t.inspect)
  end
