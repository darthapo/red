...
describe "Module#const_set" do |it| 
  it.will "set the constant with the given name to the given value" do
    Module.const_set :A, "A"
    Module.const_set "C", "C"
    
    Module.const_get("A").should_equal("A")
    Module.const_get("C").should_equal("C")
  end
  ...
  it.tries "to convert the given name to a string using to_str" do
    ...
    Module.const_get(:A).should_equal("test")
  end
  
  it.raises " a TypeError when the given name can't be converted to string using to_str" do
    o = mock('123')
    lambda { Module.const_set(o, "test") }.should_raise(TypeError)
