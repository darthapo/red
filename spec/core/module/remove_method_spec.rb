...
describe "Module#remove_method" do |it| 
  it.can "remove the method from a class" do
    x = ModuleSpecs::NoInheritance.new
    x.respond_to?(:method_to_remove).should_equal(false)
  end
  
  it.can "remove method from subclass, but not parent" do
    x = ModuleSpecs::Child.new
    x.respond_to?(:method_to_remove).should_equal(true    )
    x.method_to_remove.should_equal(1)
  end
