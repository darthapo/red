describe "Array#|" do |it|
  it.returns "an array of elements that appear in either array (union)" do
    ([] | []).should_equal([])
    ([1, 2] | []).should_equal([1, 2])
    ([] | [1, 2]).should_equal([1, 2])
    ([ 1, 2, 3, 4 ] | [ 3, 4, 5 ]).should_equal([1, 2, 3, 4, 5])
  end
  
  it.can "create an array with no duplicates" do
    ([ 1, 2, 3, 1, 4, 5 ] | [ 1, 3, 4, 5, 3, 6 ]).should_equal([1, 2, 3, 4, 5, 6])
  end
  
  it.creates "an array with elements in order they are first encountered" do
    ([ 1, 2, 3, 1 ] | [ 1, 3, 4, 5 ]).should_equal([1, 2, 3, 4, 5])
  end

  it.can "properly handle recursive arrays" do
    empty = ArraySpecs.empty_recursive_array
    (empty | empty).should_equal(empty)

    array = ArraySpecs.recursive_array
    (array | []).should_equal([1, 'two', 3.0, array])
    ([] | array).should_equal([1, 'two', 3.0, array])
    (array | array).should_equal([1, 'two', 3.0, array])
    (array | empty).should_equal([1, 'two', 3.0, array, empty])
  end

  it.will "try to convert the passed argument to an Array using #to_ary" do
    obj = mock('[1,2,3]')
    obj.should_receive(:to_ary).and_return([1, 2, 3])
    ([0] | obj).should_equal(([0] | [1, 2, 3]))
  end

  it.can "check whether the passed argument responds to #to_ary" do
    obj = mock('[1,2,3]')
    obj.should_receive(:respond_to?).with(:to_ary).any_number_of_times.and_return(true)
    obj.should_receive(:method_missing).with(:to_ary).and_return([1, 2, 3])
    ([0] | obj).should_equal([0, 1, 2, 3])
  end

  # MRI follows hashing semantics here, so doesn't actually call eql?/hash for Fixnum/Symbol
  it.can "act as if using an intermediate hash to collect values" do
    ([5.0, 4.0] | [5, 4]).should_equal([5.0, 4.0, 5, 4])
    str = "x"
    ([str] | [str.dup]).should_equal([str])
    
    obj1 = mock('1')
    obj2 = mock('2')
    def obj1.hash; 0; end
    def obj2.hash; 0; end
    def obj1.eql? a; true; end
    def obj2.eql? a; true; end
    
    ([obj1] | [obj2]).should_equal([obj1])
    
    def obj1.eql? a; false; end
    def obj2.eql? a; false; end
    
    ([obj1] | [obj2]).should_equal([obj1, obj2])
  end
  
  it.does_not "return subclass instances for Array subclasses" do
    (ArraySpecs::MyArray[1, 2, 3] | []).class.should_equal(Array)
    (ArraySpecs::MyArray[1, 2, 3] | ArraySpecs::MyArray[1, 2, 3]).class.should_equal(Array)
    ([] | ArraySpecs::MyArray[1, 2, 3]).class.should_equal(Array)
  end
  
  it.does_not "call to_ary on array subclasses" do
    ([1, 2] | ArraySpecs::ToAryArray[5, 6]).should_equal([1, 2, 5, 6])
  end
end
