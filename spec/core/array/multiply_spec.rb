...
describe "Array#*" do |it| 
  it.is "equivalent to self.join(str) when passed a string" do
    ([ 1, 2, 3 ] * ",").should_equal([1, 2, 3].join(","))
  end

  it.can "handle recursive arrays like #join" do
    x = []
    x << x
    (x * ":").should_equal(x.join(":"))
  ...
  it.can "properly handles recursive arrays" do
    ...
    (empty * 3).should_equal([empty, empty, empty])
    (empty * ",").should_equal("[...]")

    array = [1, 2]; array << array
    (array * ",").should_equal("1,2,1,2,[...]")
    (array * 0).should_equal([])
  end

  it.calls " to_str on its argument" do
    obj = mock('x')
    def obj.to_str() "x" end
    ([1, 2, 3] * obj).should_equal([1, 2, 3].join("x"))
  end
  ...
  it.can "concatenates n copies of the array when passed an integer" do
    ([ 1, 2, 3 ] * 0).should_equal([])
    ([ 1, 2, 3 ] * 1).should_equal([1, 2, 3])
    ([ 1, 2, 3 ] * 3).should_equal([1, 2, 3, 1, 2, 3, 1, 2, 3])
    ([] * 10).should_equal([])
  end
  
  it.raises " an ArgumentError when passed a negative integer" do
    lambda { [ 1, 2, 3 ] * -1 }.should_raise(ArgumentError)
  ...
  it.calls " to_int on its argument" do
    ...
    ([1, 2, 3] * obj).should_equal([1, 2, 3] * obj.to_int)
  end
  ...
  it.checks "whether the passed argument responds to #to_int" do
    ...
    ([1, 2, 3] * obj).should_equal([1, 2, 3] * 2)
  end
  ...
  it.calls " to_str on its argument before to_int" do
    ...
    def obj.to_int() 2 end
    def obj.to_str() "x" end
    ([1, 2, 3] * obj).should_equal([1, 2, 3] * obj.to_str)
  end

  it.returns "subclass instance with Array subclasses" do
    (ArraySpecs::MyArray[1, 2, 3] * 0).class.should_equal(ArraySpecs::MyArray)
    (ArraySpecs::MyArray[1, 2, 3] * 1).class.should_equal(ArraySpecs::MyArray)
    (ArraySpecs::MyArray[1, 2, 3] * 2).class.should_equal(ArraySpecs::MyArray)
  end

  it.raises " a TypeError if the argument can neither be converted to a string nor an integer" do
    lambda { [1, 2] * mock('str') }.should_raise(TypeError)
  end  
end
