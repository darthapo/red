...
describe "Array.new" do |it| 
  ...
  it.tries "to convert the passed arguments to Integers using #to_int when they don't respond to #to_ary" do
    ...
    Array.new(obj).should_equal([nil, nil, nil])
  end
  ...
  it.returns "a new array of size default objects" do
    Array.new(4, true).should_equal([true, true, true, true])
    ...
    a.class.should_equal(ArraySpecs::MyArray)
    a.inspect.should_equal([true, true, true, true].inspect)
  end
  
  it.does_not "call to_ary on Array subclasses when passed an array-like argument" do
    Array.new(ArraySpecs::ToAryArray[5, 6, 7]).should_equal([5, 6, 7])
  end
  ...
  it.calls " to_ary on an argument before to_int" do
    ...
    Array.new(obj).should_equal([1, 2, 3])
  end
    
  it.returns "an array of size elements from the result of passing each index to block" do
    Array.new(5) { |i| i + 1 }.should_equal([1, 2, 3, 4, 5])
    
    a = ArraySpecs::MyArray.new(5) { |i| i + 1 }
    a.class.should_equal(ArraySpecs::MyArray)
    a.inspect.should_equal([1, 2, 3, 4, 5].inspect)
  end  

  it.can "will fail if a to_ary is supplied as the first argument and a second argument is given" do
    lambda { Array.new([1, 2], 1) }.should_raise(TypeError)
  end
end
