...
describe "Class.inherited" do |it| 
  ...
  it.is "invoked with the child Class when self is subclassed" do
    begin
      ...
      child = Class.new(top)
      $child_class.should_equal(child)

      other_child = Class.new(top)
      $child_class.should_equal(other_child)
    ensure
      $child_class = nil
    end
  end
  ...
  it.is "invoked only once per subclass" do
    ...
    ClassSpecs::Inherited::A::SUBCLASSES.should_equal(expected)
  end

  it.is "called when marked as a private class method" do
    ClassSpecs::A.private_class_method :inherited
    ClassSpecs::Record.called?.should_equal(nil)
    module ClassSpecs; class B < A; end; end
    ClassSpecs::Record.called?.should_equal(ClassSpecs::B)
  end
  ...
  it.is "called when marked as a protected class method" do
    ...
    ClassSpecs::Record.called?.should_equal(ClassSpecs::C)
  end
  ...
  it.is "called by super even when marked as a private class method" do
    ...
    ClassSpecs::Record.called?.should_equal(ClassSpecs::I)
  end
  
  it.can "will be invoked by child class regardless of visibility" do
    top = Class.new do
      class << self
