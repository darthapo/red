...
describe "Hash#delete" do |it| 
  it.can "removes the entry and returns the deleted value" do
    h = {:a => 5, :b => 2}
    h.delete(:b).should_equal(2)
    h.should_equal({:a => 5})
  end
  ...
  it.calls " eql? on colliding keys it finds" do
    ...
    (obj1.eql_count + obj2.eql_count).should_equal(2)
  end

  it.calls " supplied block if the key is not found" do
    {:a => 1, :b => 10, :c => 100 }.delete(:d) { 5 }.should_equal(5)
    Hash.new(:default).delete(:d) { 5 }.should_equal(5)
    Hash.new() { :defualt }.delete(:d) { 5 }.should_equal(5)
  end
  
  it.returns "nil if the key is not found when no block is given" do
    {:a => 1, :b => 10, :c => 100 }.delete(:d).should_equal(nil)
    Hash.new(:default).delete(:d).should_equal(nil)
    Hash.new() { :defualt }.delete(:d).should_equal(nil)
  end
end
