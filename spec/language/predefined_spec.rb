...
describe "Predefined global $~" do |it| 
  it.is "set to contain the MatchData object of the last match if successful" do
    md = /foo/.match 'foo'
    $~.class.should_equal(MatchData)
    $~.object_id.should_equal(md.object_id)
  ...
  it.raises " an error if assigned an object not nil or instanceof MatchData" do
    ...
    lambda { $~ = 1 }.should_raise(TypeError)
  end
end
...
describe "Predefined global $+" do |it| 
  it.is "equivalent to $~.captures.last" do
    /(f(o)o)/ =~ 'barfoobaz'
    $+.should_equal($~.captures.last)
    $+.should_equal('o')
  end
end

describe "Predefined globals $1..N" do |it| 
  it.can "are equivalent to $~[N]" do
    /(f)(o)(o)/ =~ 'foo'
    $1.should_equal($~[1])
    $2.should_equal($~[2])
    $3.should_equal($~[3])
    ...
    [$1, $2, $3, $4].should_equal(['f', 'o', 'o', nil])
  end

  it.can "are nil unless a match group occurs" do
    def test(arg)
      case arg
      when /-(.)?/
        $1
    ...
    test("-").should_equal(nil)
  end
end

describe "Predefined global $stdout" do |it| 
  it.before(:each) do
    @old_stdout = $stdout
  end
  ...
  it.is "the same as $DEFAULT_OUTPUT from 'English' library" do
    require 'English'
    $stdout.should_equal($DEFAULT_OUTPUT)
  ...
  it.raises " TypeError error if assigned to nil" do
    lambda { $stdout = nil }.should_raise(TypeError)
  end

  it.raises " TypeError error if assigned to object that doesn't respond to #write" do
    obj = mock('object')
    lambda { $stdout = obj }.should_raise(TypeError)
...
describe "Predefined global $_" do |it| 
  it.is "set to the last line read by e.g. StringIO#gets" do
    stdin = StringIO.new("foo\nbar\n", "r")

    read = stdin.gets
    read.should_equal("foo\n")
    $_.should_equal(read)

    read = stdin.gets
    read.should_equal("bar\n")
    $_.should_equal(read)

    read = stdin.gets
    read.should_equal(nil)
    $_.should_equal(read)
  end
  ...
  it.is "set at the method-scoped level rather than block-scoped" do
    ...
    match.should_equal("bar\n")
    $_.should_equal(match)

    eval 'match = stdin.gets'

    match.should_equal("baz\n")
    $_.should_equal(match)

    obj.foo2 { match = stdin.gets }

    match.should_equal("qux\n")
    $_.should_equal(match)
  end

  it.can "can be assigned any value" do
...
=end
describe "Execution variable $:" do |it| 
  it.is "initialized to an array of strings" do
    $:.is_a?(Array).should_equal(true)
    ($:.length > 0).should_equal(true)
  end

  it.can "includes the current directory" do
    $:.should_include(".")
  end

  it.does_not "include on the taint check level > 1" do
    begin
      orig_opts, ENV['RUBYOPT'] = ENV['RUBYOPT'], '-T'
      `#{RUBY_EXE} -e 'p $:.include?(".")'`.should_equal("false\n")
    ensure
  ...
  it.is "the same object as $LOAD_PATH and $-I" do
    $:.__id__.should_equal($LOAD_PATH.__id__)
    $:.__id__.should_equal($-I.__id__)
  end
  
  it.can "can be changed via <<" do
    $: << "foo"
    $:.should_include("foo")
  end
end
...
describe "The predefined standard objects" do |it| 
  it.can "includes ARGF" do
    Object.const_defined?(:ARGF).should_equal(true)
  end
  
  it.can "includes ARGV" do
    Object.const_defined?(:ARGV).should_equal(true)
  end
  
  it.can "includes a hash-like object ENV" do
    Object.const_defined?(:ENV).should_equal(true)
    ENV.respond_to?(:[]).should_equal(true)
  end
end

describe "The predefined standard object nil" do |it| 
...
describe "The predefined standard object true" do |it| 
...
describe "The predefined standard object false" do |it| 
  it.is "an instance of FalseClass" do
    false.class.should_equal(FalseClass)
  end
...
describe "The predefined global constants" do |it| 
  it.can "includes DATA when main script contains __END__" do
    ...
    ruby.close_write
    ruby.gets.chomp.should_equal('true')
  end

  it.does_not "include DATA when main script contains no __END__" do
    ruby = IO.popen(RUBY_NAME, "w+")
    ruby.puts("puts Object.const_defined?(:DATA)")
    ruby.close_write
    ruby.gets.chomp.should_equal('false')
  end

  it.can "includes TRUE" do
    Object.const_defined?(:TRUE).should_equal(true)
    TRUE.should_equal(true)
  end
  
  it.can "includes FALSE" do
    Object.const_defined?(:FALSE).should_equal(true)
    FALSE.should_equal(false)
  end
  
  it.can "includes NIL" do
    Object.const_defined?(:NIL).should_equal(true)
    NIL.should_equal(nil)
  end
  
  it.can "includes STDIN" do
    Object.const_defined?(:STDIN).should_equal(true)
  end
  
  it.can "includes STDOUT" do
    Object.const_defined?(:STDOUT).should_equal(true)
  end
  
  it.can "includes STDERR" do
    Object.const_defined?(:STDERR).should_equal(true)
  end

  it.can "includes RUBY_VERSION" do
    Object.const_defined?(:RUBY_VERSION).should_equal(true)
  end
  
  it.can "includes RUBY_RELEASE_DATE" do
    Object.const_defined?(:RUBY_RELEASE_DATE).should_equal(true)
  end
  
  it.can "includes RUBY_PLATFORM" do
    Object.const_defined?(:RUBY_PLATFORM).should_equal(true)
  end

  it.can "includes PLATFORM" do
    Object.const_defined?(:PLATFORM).should_equal(true)
    RUBY_PLATFORM == PLATFORM
  end

  it.can "includes TOPLEVEL_BINDING" do
    Object.const_defined?(:TOPLEVEL_BINDING).should_equal(true)
  end
end
